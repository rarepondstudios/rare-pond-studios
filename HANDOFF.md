...
- **Root cause.** The pathname did not reflect the on-screen view after a `#hash` arrival, so the guard's same-page test was wrong.
- **Fix (at the source).** `routeHash()` on the studio and jackcarlsen now `history.replaceState()`s the canonical path as it shows the view (`#team -> /team`, `#projects -> /projects`; jackcarlsen `#projects|#portfolios -> /portfolios`, `#<role> -> /<role>`) and drops the hash. Home/scroll hashes (`#about`, `#contact`) are untouched. The pathname now matches the view, so the guard is correct AND the wordmark is a real `/team -> /` navigation that runs the normal `__pageFade` transition, identical to an in-app home -> subpage click. No new global hooks; the shared SAME-PAGE NAV guard is unchanged from its original form.
- **Also this session.** Removed the three glowing-dot `tag-badge` bubbles ("Why us?", "How we work", "What sets us apart") from the media page (`media/index.html`). Section headings and copy untouched; the CMS `sections[].badge` field is now inert (the JS injection guard is a no-op with the span gone). The `.tag-badge` CSS is left in place, unused and harmless.
- **Deploy.** `rp_site_work`: stage the changed paths only (never `git add -A`) and push. `jackcarlsen-website` pushes from the mini ONLY, as `Jackjrrc`, then switch the active GitHub account back to `rarepondstudios`.
