...
(function () {
  ...
  function bannerCssOnce() {
    ...
    var css = [
      ...
      // content sits in the visible colour band, above sheet+glow. Anchored by its BOTTOM (not top) so that
      // on narrow screens — where the title + button wrap to two lines — the block grows UPWARD into the
      // coloured band instead of hanging DOWN out of it. Single-line desktop is unchanged (bottom edge at
      // 150px == old top:118px + 32px height), and --evbanner-h stays constant since the bottom is fixed.
      ".rp-evbanner .rp-evinner{position:absolute;left:0;right:0;top:auto;bottom:20px;z-index:3;max-width:1180px;margin:0 auto;display:flex;align-items:center;justify-content:center;gap:10px 22px;flex-wrap:wrap;padding:0 clamp(18px,5vw,60px);text-align:center}",
