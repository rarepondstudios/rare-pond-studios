...
`in_range=tv` is the part that matters. Get it wrong and blacks sit at ~16 instead of ~0 - the image looks milky and lifted. (`out_range` is a red herring here: RGB output is always
...
shot*. On `geri-nursing-1` this picked a frame with a nurse walking through in motion blur - a person who is not in the real still. Widening to **±12 s** found the right frame, 12× better
